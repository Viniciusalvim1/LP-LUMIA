...
const steps: Step[] = [
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" {...ic} aria-hidden="true">
  ...
  {
    title: "Início do processo de migração",
...
export default function OnboardingSection() {
  ...
  // Scroll-linked: a estrela percorre a linha conforme a seção entra na tela
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start 70%", "end 65%"],
  });

  // Posição vertical da estrela e altura da trilha (0% → 100%)
  const trailHeight = useTransform(scrollYProgress, [0, 1], ["0%", "100%"]);

  // Acende os nós conforme a estrela passa
  useMotionValueEvent(scrollYProgress, "change", (p) => {
    const idx = Math.min(steps.length - 1, Math.floor(p * steps.length));
    setActiveIndex(p <= 0 ? -1 : idx);
  });
    ...
    <section className="relative py-20">
      <div className="relative z-10 max-w-[1100px] mx-auto px-5 lg:px-12">
        ...
        <div className="text-center mb-16">
          ...
          <h2
            className="text-[30px] md:text-[40px] font-semibold leading-[1.15] text-white mb-4"
            style={{ fontFamily: "var(--font-display)" }}
          >
            A gente implementa{" "}
            <span
          ...
          <p
            className="text-[17px] leading-[1.7] text-white/60 max-w-[560px] mx-auto"
            style={{ fontFamily: "var(--font-display)" }}
          >
        ...
        <div ref={containerRef} className="relative">
          ...
          {/* Trilha luminosa (cresce com o scroll) */}
          <motion.div
            className="absolute top-0 left-[27px] lg:left-1/2 w-px -translate-x-1/2"
            style={{
              height: trailHeight,
          ...
          >
            {/* Cabeça da estrela cadente */}
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2">
              <div
                className="w-3 h-3 rounded-full bg-white"
                style={{ boxShadow: "0 0 8px 2px rgba(255,255,255,0.9), 0 0 20px 6px rgba(110,245,208,0.7)" }}
              />
            </div>
          </motion.div>
          ...
          <div className="flex flex-col gap-10 lg:gap-14">
            {steps.map((step, i) => {
              ...
              const onLeft = i % 2 === 0; // par à esquerda no desktop

              return (
                <div
                  key={step.title}
                  className="relative pl-[68px] lg:pl-0 lg:grid lg:grid-cols-2 lg:gap-16 lg:items-center"
                >
                  {/* Nó numerado */}
                  ...
                  >
                    <div
                      className={`inline-flex items-center gap-3 mb-3 ${onLeft ? "lg:flex-row-reverse" : ""}`}
                    >
                      <span
